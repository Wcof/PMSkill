...
- `/prd-init`、`/prd-start`、`/prd-status`、`/prd-remove` 的提示必须跟随上述语言策略。

在项目中使用前，先安装完整的 `prd-helper` Skill。

## Activation Rule

当用户触发 `/prd-init`，或 Agent 发现当前项目已安装但尚未初始化时，必须先自动初始化项目，不要只提示用户再去运行别的命令。
...
按顺序尝试以上路径，使用第一个存在的脚本。初始化完成后，告诉用户下一步发送 `/prd-start` 开启采集。
...
Agent 应自动初始化当前项目，默认使用 `docs/prd-helper/`、启用已安装 Agent，并设置采集策略为只通过 `/prd-start` 显式开启。
...
| Command | Action |
|---------|--------|
| `/prd-init` | 初始化当前项目：创建 docs 根目录、写入配置、准备采集目录、生成命令文件。 |
...
当用户发送 `/prd-init` 且项目尚未初始化时，自动执行已安装 Skill 目录中的 setup 脚本。按顺序尝试以下路径，使用第一个存在的路径：
