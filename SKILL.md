...
- 如果当前 Agent 是 Claude Code，还必须写入 `.claude/settings.json` 中的 `UserPromptSubmit` 与 `Stop` hooks，让 `/prd-start` 后的会话轮次自动落盘。
...
| `/prd-start` | 开启 PRD 采集会话。创建 `collect-state.md`、`active/`、`passive/`、`source-index.md`，设置 `capture_mode: on`，并依赖 Claude Code hooks 自动采集后续完整会话轮次。 |
