...
当用户触发 `/prd-helper`，或 Agent 发现当前项目已安装但尚未初始化时，必须先自动初始化项目，不要只提示用户再去运行别的命令。
...
按顺序尝试以上路径，使用第一个存在的脚本。初始化完成后，告诉用户下一步可以使用 `/prd-start`；如果当前平台不显示该命令，则使用兼容入口 `/prd-helper start`。

当用户使用 `/prd-helper <action>` 兼容入口时，按以下方式执行：

```bash
python3 scripts/collect-control.py start --root docs/prd-helper/01-collect --agent <current-agent>
python3 scripts/collect-control.py pause --root docs/prd-helper/01-collect --agent <current-agent>
python3 scripts/collect-control.py resume --root docs/prd-helper/01-collect --agent <current-agent>
python3 scripts/collect-control.py stop --root docs/prd-helper/01-collect --agent <current-agent>
python3 scripts/collect-control.py status --root docs/prd-helper/01-collect --agent <current-agent>
```

在已安装 Skill 中运行时，优先使用已安装目录下的脚本路径，例如 `.claude/skills/prd-helper/scripts/collect-control.py` 或 `.agents/skills/prd-helper/scripts/collect-control.py`。
...
安装完成后，在 Agent 中运行总入口：

```text
/prd-helper
```

Agent 应自动初始化当前项目，默认使用 `docs/prd-helper/`、启用已安装 Agent，并设置采集策略为只通过 `/prd-start` 显式开启。需要覆盖默认配置时，用户仍可使用 `/prd-setup`。
...
| `/prd-helper start` | 兼容入口：当平台只显示 `/prd-helper` 时，用它执行 `/prd-start`。 |
| `/prd-helper pause` | 兼容入口：暂停采集。 |
| `/prd-helper resume` | 兼容入口：恢复采集。 |
| `/prd-helper stop` | 兼容入口：停止采集。 |
| `/prd-helper status` | 兼容入口：查看采集状态。 |
...
当用户发送 `/prd-helper` 且项目尚未初始化时，自动执行已安装 Skill 目录中的 setup 脚本。按顺序尝试以下路径，使用第一个存在的路径：
...
当用户发送 `/prd-setup` 时，执行同一初始化脚本；如果用户明确要求自定义 docs 目录或 Agent 范围，再询问配置项。
