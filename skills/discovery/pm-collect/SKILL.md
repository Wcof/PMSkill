---
name: pm-collect
description: 主动深度扫描 URL、项目、知识库和对话上下文，收集零散想法、用户反馈、会议纪要、历史会话等材料。支持项目深扫描：代码目录结构、git commits、TODO/FIXME、配置文件全量扫描。Use when materials are needed before refining, or the user mentions feedback, meeting notes, or raw product context.
---
...
**Philosophy**：collect 只整理不改写——原样聚合材料并建立关联，事实提取留给 refine。宁可材料多而杂，也不要遗漏可能的关键线索。
...
**🔴 CHECKPOINT**：collect 完成后若材料总量 < 5 条，提示 PM"当前材料较少（仅 N 条），建议补充 URL 引用或粘贴更多上下文后再进入 refine 阶段"。PM 可决定继续或补充。
