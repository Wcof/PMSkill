...
**Philosophy**：给人看的 PRD 答的是"为什么这么定"而非"怎么做"——决策理由比结论更重要、自然语言叙事比 As-an 模板更高效、表格只放 3-5 行关键信息超过的拆为列表。
...
- 使用分段式标题（H2/H3），便于评审者快速扫描
- 每个用户故事前加 emoji 标记视觉区分：🎯 用户故事
- 决策理由放在每条规则后的 `> **为什么**` 引用块中
- 避免表格过密——给人看的东西，表格只放 3-5 行关键信息，超过的用列表
...
| 反模式 | 为什么不要做 |
|--------|------------|
| 塞入 Agent Context、技术栈细节等机器可读内容 | 人类评审者不需要，反而造成干扰 |
| 用英文 As-an/I-want 模板写用户故事 | 中文团队评审，自然语言叙事更高效 |
