...
- **PRD Discuss**: `/prd-discuss`（原 `/prd-grill`），在 Active Capture 基础上开启需求研讨模式。扫描已采集材料找矛盾，逐个问题压力测试产品方案，实时更新 CONTEXT.md，按需创建 ADR。
...
- **命令集合**: `/prd-helper`、`/prd-start`、`/prd-stop`、`/prd-status`、`/prd-scan`、`/prd-import`、`/prd-refine`、`/prd-relate`、`/prd-generate`、`/prd-discuss`、`/prd-remove`。
